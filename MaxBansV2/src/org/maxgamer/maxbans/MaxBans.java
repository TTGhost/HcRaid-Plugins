 ...
 * <br/>
 * <b>Server Website</b> http://maxgamer.org/<br/>
 * </a> <b>Primary Server IP</b> play.maxgamer.org<br/>
 * <b>Source code</b> http://github.com/netherfoam/MaxBans/<br/>
 * <b>Compiled plugin</b> http://dev.bukkit.org/server-mods/maxbans/<br/>
 * <br/>
 * 
 * @author Netherfoam, Darekfive<br/>
 * <br/>
 */
public class MaxBans extends JavaPlugin {
	...
	public void onEnable() {
		...
		/*
		 * Reloads the config from disk. Normally this is done before onEnable()
		 * anyway, but, if we do /plugman reload MaxBans, it doesnt. This makes
		 * it friendlier.
		 */
		this.reloadConfig();
		Msg.reload();
			...
			Runnable download = new Runnable() {
				...
				public void run() {
					...
					try {
						...
						e.printStackTrace();
						System.out
								.println("Failed to download MaxBans GeoIPDatabase");
					}
				}
			};
		...
		ConfigurationSection dbConfig = getConfig().getConfigurationSection(
				"database");
		...
		try {
			...
			e1.printStackTrace();
			System.out
					.println("Failed to create connection to database. Disabling MaxBans :(");
			getServer().getPluginManager().disablePlugin(this);
			return;
		}

		ConfigurationSection syncConfig = this.getConfig()
				.getConfigurationSection("sync");
		...
		Bukkit.getServer().getPluginManager()
				.registerEvents(this.chatListener, this);

		this.joinListener = new JoinListener();
		this.chatCommandListener = new ChatCommandListener();

		Bukkit.getServer().getPluginManager()
				.registerEvents(this.joinListener, this);
		Bukkit.getServer().getPluginManager()
				.registerEvents(this.chatCommandListener, this);
		...
		if (this.isBungee()) {
			...
			Bukkit.getMessenger().registerIncomingPluginChannel(this,
					MaxBans.BUNGEE_CHANNEL, new BungeeListener());
			// Outgoing (Requests for IPs)
			Bukkit.getMessenger().registerOutgoingPluginChannel(this,
					MaxBans.BUNGEE_CHANNEL);
		}
	}
	...
	public void onDisable() {
		...
		if (syncer != null) {
			syncer.stop();
			syncer = null; // Required when reloading, if sync.use changes to
							// false
		}

		if (syncServer != null) {
			syncServer.stop();
			syncServer = null; // Required when reloading, if sync.server
								// changes to false
		}
