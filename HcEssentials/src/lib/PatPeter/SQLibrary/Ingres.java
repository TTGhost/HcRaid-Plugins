...
public class Ingres extends Database {
	...
	public Ingres(Logger log, String prefix, String hostname, int port,
	...
	public boolean initialize() {
		try {
			...
			this.writeError("Ingres driver class missing: " + e.getMessage()
					+ ".", true);
			return false;
		}
	}
	...
	public boolean open() {
		if (initialize()) {
			String url = "";
			url = "jdbc:ingres://" + getHostname() + ":" + getPort() + "/"
					+ getDatabase();
			try {
				...
				this.writeError(
						"Could not establish a Ingres connection, SQLException: "
								+ e.getMessage(), true);
				return false;
			}
		} else {
