...
public class DB2 extends Database {
	...
	public DB2(Logger log, String prefix, String database) {
	...
	public DB2(Logger log, String prefix, String hostname, int port,
	...
	public DB2(Logger log, String prefix, String database, String username) {
		super(log, prefix, "[DB2] ");
		setHostname("localhost");
		setPort(523);
		setDatabase(database);
	...
	protected boolean initialize() {
		try {
			...
			this.writeError(
					"DB2 driver class missing: " + e.getMessage() + ".", true);
			return false;
		}
	}
	...
	public boolean open() {
		if (initialize()) {
			String url = "jdbc:derby:net://" + getHostname() + ":" + getPort()
					+ "/" + getDatabase();
			try {
				...
				this.writeError(
						"Could not establish a DB2 connection, SQLException: "
								+ e.getMessage(), true);
				return false;
			}
		} else {
