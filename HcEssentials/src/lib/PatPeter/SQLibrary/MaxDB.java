...
public class MaxDB extends Database {
	...
	public MaxDB(Logger log, String prefix, String database) {
	...
	public MaxDB(Logger log, String prefix, String hostname, int port,
	...
	public MaxDB(Logger log, String prefix, String database, String username) {
		super(log, prefix, "[MaxDB] ");
		setHostname("localhost");
		setPort(7210);
		setDatabase(database);
	...
	protected boolean initialize() {
		try {
			...
			this.writeError("MaxDB driver class missing: " + e.getMessage()
					+ ".", true);
			return false;
		}
	}
	...
	public boolean open() {
		if (initialize()) {
			String url = "jdbc:sapdb://" + getHostname() + ":" + getPort()
					+ "/" + getDatabase();
			try {
				...
				this.writeError(
						"Could not establish a MaxDB connection, SQLException: "
								+ e.getMessage(), true);
				return false;
			}
		} else {
