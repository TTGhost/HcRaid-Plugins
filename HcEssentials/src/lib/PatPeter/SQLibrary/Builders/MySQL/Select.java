...
public class Select implements Builder {
		...
		public static Duplicates byID(int id) throws BuilderException {
			if (id < 0 || 2 < id)
				throw new BuilderException(
						"Duplicates must be between 0 and 2.");
	...
	/*
	 * public Select(Database db, String columns, String tables) throws
	 * DatabaseException { //setDatabase(db); setColumns(columns);
	 * setTables(tables); }
	 */
	...
	private boolean checkConditional(String conditional) {
		...
		for (String c : conditionals)
			if (conditional.equals(c))
				return true;
		db.writeError("Skipping unknown conditional " + conditional + ".",
				false);
		return false;
	}
		...
		for (String column : columns) {
			if (column != null && column.length() != 0) {
				if (!column.contains("`")) {
					...
					db.writeError("Column " + column
							+ " in SELECT statement cannot have backticks.",
							false);
				}
			} else {
				db.writeError("Column at position " + counter
						+ " cannot be null or empty in SELECT statement.",
						false);
			}
			counter++;
		}
	...
	public ResultSet execute() throws SQLException {
		if (columns.isEmpty())
			throw new BuilderException(
					"Must specify at least one column in a SELECT statement.");
	...
	public Select groupBy(String expression) {
		if (!validString(expression,
				"Skipping null or empty GROUP BY expression."))
			return this;
	...
	public Select having(String conditional, String condition) {
		if (having.size() != 0) {
			...
			db.writeError("Cannot add conditional " + conditional
					+ " to the front of a HAVING statement.", false);
		}
		if (!checkCondition(condition))
			return this;
	...
	public Select orderBy(String expression) {
		if (!validString(expression,
				"Skipping null or empty ORDER BY expression."))
			return this;
	...
	private void setDatabase(Database db) throws DatabaseException {
		if (db == null)
			throw new DatabaseException(
					"Database cannot be null in SELECT statement.");
	...
	/*
	 * private void setColumns(String columns) throws DatabaseException { if
	 * (columns == null || columns.isEmpty()) throw new
	 ...
	 * DatabaseException
	 * ("Invalid syntax in SELECT query. Two backticks must surround column " +
	 * t + ".");
	 * 
	 * this.columns = Arrays.asList(temp); } else { if (columns.split("`",
	 * -1).length - 1 == 2) throw new DatabaseException(
	 * "Invalid syntax in SELECT query. Two backticks must surround column " +
	 * columns + ".");
	 * 
	 * this.columns.set(0, columns); } }
	 * 
	 * private void setTables(String tables) throws DatabaseException { if
	 * (tables == null || tables.isEmpty()) throw new
	 ...
	 * for (String t : temp) if (t.split("`", -1).length - 1 == 2) throw new
	 * DatabaseException
	 * ("Invalid syntax in SELECT query. Two backticks must surround table " + t
	 * + ".");
	 * 
	 * this.tables = Arrays.asList(temp); } else { if (tables.split("`",
	 * -1).length - 1 == 2) throw new DatabaseException(
	 * "Invalid syntax in SELECT query. Two backticks must surround table " +
	 * tables + ".");
	 * 
		...
		for (String table : tables) {
			if (table != null && !table.isEmpty()) {
				if (!table.contains("`")) {
					...
					db.writeError("Skipping table " + table
							+ " in SELECT statement that has backticks.", false);
				}
			} else {
				db.writeError("Skipping table in SELECT statement at position "
						+ counter + " for being null or empty.", false);
			}
			counter++;
		}
		return this;
	...
	public String toString() {
		...
		if (!tables.isEmpty()) {
			...
			string += (update != null ? (update ? "FOR UPDATE"
					: "LOCK IN SHARE MODE") : "");
		}
	...
	public Select where(String conditional, String condition) {
		if (where.size() != 0) {
			...
			db.writeError("Cannot add conditional " + conditional
					+ " to the front of a WHERE statement.", false);
		}
		if (!checkCondition(condition))
			return this;
