...
public class Informix extends Database {
	...
	public Informix(Logger log, String prefix, String database) {
	...
	public Informix(Logger log, String prefix, String hostname, int port,
	...
	public Informix(Logger log, String prefix, String database, String username) {
		super(log, prefix, "[Informix] ");
		setHostname("localhost");
		setPort(1526);
		setDatabase(database);
	...
	protected boolean initialize() {
		try {
			...
			this.writeError("Informix driver class missing: " + e.getMessage()
					+ ".", true);
			return false;
		}
	}
	...
	public boolean open() {
		if (initialize()) {
			String url = "jdbc:informix-sqli://" + getHostname() + ":"
					+ getPort() + ":informixserver=" + getDatabase() + ";";
			try {
				...
				this.writeError(
						"Could not establish a Informix connection, SQLException: "
								+ e.getMessage(), true);
				return false;
			}
		} else {
